...
alter table population rename city;
alter table city change id  id varchar(10);
delete from city where id = '6';
alter table city change countrycode countrycode varchar(30);
insert into city values ('6','Rotterdam','NLD','Zuid-Holland','120934');
...
select state from station group by state;
...
select distinct city from station where city regexp "^[aieouAEIOU]" order by city;
/* Another way */
...
select distinct city from station where city  not regexp '[aieouAEIOU]%';
/* either do not start with vowel or do not end with vowels*/
select distinct city from station where city not regexp '^[aieouAEIOU]' or city not regexp '[aeiouAIEOU]$'order by city;
/* neither start with vowel nor end with vowels*/
select distinct city from station where city not regexp '^[aeiouAEIOU]' and city not regexp '[aieouAEIOU]$' order by city;
